        ...
        <div className="flex justify-between items-center">
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex items-center gap-2 group">
              <Camera className="h-8 w-8 text-amber-500 group-hover:text-amber-400 transition-colors" />
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-stone-300 hover:text-amber-400 px-3 py-2 text-sm font-medium tracking-wide transition-colors"
              >
                {link.name}
              </a>
            ))}
            <a 
              href="#contact"
              className="bg-amber-600 hover:bg-amber-500 text-white px-5 py-2 rounded-none text-sm font-medium tracking-wide transition-colors uppercase"
            >
              Book Session
            </a>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-stone-300 hover:text-amber-400 block px-3 py-2 text-base font-medium"
